...
export const initializeMap = async (
  doctors: IDoctorDetails[] | IFavouriteDoctor[],
  onMarkerClick: (
    doctor:
      | (IDoctorDetails & { lat: number; lng: number })
      | (IFavouriteDoctor & { lat: number; lng: number })
  ) => void
): Promise<L.Map | null> => {
  try {
    ...
    for (let i = 0; i < doctors.length; i++) {
      ...
      if (coords.lat && coords.lng && coords.lat !== 0 && coords.lng !== 0) {
        ...
        doctorMarker.bindPopup(
          `<b>دكتور: ${doctor.name}</b><br>${doctor.address ?? ""}`
        );
...
export const cleanupMap = (map: L.Map) => {
  if (map) {
    map.remove();
  }
};
